...
const CourseInfo = {
  ...
  const AssignmentGroup = {
    ...
    assignments: [
      ...
      {
        //this assignment will not count in the
        //avg as it is not due.
        id: 3,
        name: "Code the World",
        due_at: "3156-11-15",
        points_possible: 500
  ...
  function CourseAvailability (assignmentGroup, courseInfo) {
    if (assignmentGroup.course_id === courseInfo.id) {
      ...
      console.log('Error: Course ID does not match. Course is not available.');
      return false;
  }
  }
  ...
  function unqLearnerIdArray ([learnerSubmissions]){
    ...
    for (let i = 0; i <learnerSubmissions.length; i++){
      if (unqLearnerId !== learnerSubmissions[i].learner_id){
        ...
        getunqLearnerId.push(unqLearnerId);
      }
